...
#    template = prompts[args.template_name]
    ...
    def preprocess_function(examples):
        ...
        for i in range(bs):
            input_texts.append(examples['inputs'][i])
            target_texts.append(examples['targets'][i][0])
            answer_choices_texts.append(examples['multiple_choice_targets'][i])
    ...
    with accelerator.main_process_first():
        print('raw_datasets:', vars(raw_datasets))
    ...
    for batch in eval_dataloader:
        ...
        preds = accelerator.gather(predictions)
        refs = accelerator.gather(batch["targets"])
        results['predictions'].extend(preds.tolist())
        results['targets'].extend(refs.tolist())
    ...
    results["evaluation"] = eval_metric
